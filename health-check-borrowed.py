...
class Api_Loader(object):
    ...
    def __init__(self):
        ...
        self.MAX_RETRY_ERROR_MSG = (
            "ERROR: Failed to establish connection - Make sure the hostname is correct"
        )
        ...
        self.SEARCH_FILTER = deepsecurity.SearchFilter(
            max_items=self.MAX_ITEMS_PER_PAGE, search_criteria=search_criteria
        )
        ...
        try:
            ...
            print(
                Fore.LIGHTRED_EX
                + "Error while loading the config/api_config.yml file, resetting it..."
            )
            try:
                os.makedirs("config")
            except Exception as e:
        ...
        try:
            if (
                "host" not in file_config
                or file_config["host"]
                == "https://<Your DSM Hostname or IP>:<DSM Port>/api"
                or "api-secret-key" not in file_config
                or file_config["api-secret-key"] == ""
                or "api-version" not in file_config
            ):
                print(Fore.LIGHTRED_EX + "CONFIG FILE NOT SET!")
                print(
                    "{}Insert the DSM host (link) following this example {}[{}https://{}<Your DSM Hostname or IP>{}:{}<DSM Port if on-premise>{}/api{}]".format(
                        Fore.LIGHTCYAN_EX,
                        Fore.LIGHTWHITE_EX,
                        Fore.LIGHTBLUE_EX,
                        Fore.LIGHTGREEN_EX,
                ...
                print(
                    Fore.LIGHTCYAN_EX
                    + "Insert the secret key for the API (Check the documentation if lost)"
                )
                self.api_config.api_key["api-secret-key"] = (
                    input("Inset the Api Secret key: ").rstrip().lstrip()
                )
                file_config["api-secret-key"] = self.api_config.api_key[
                    "api-secret-key"
                ]
                ...
                print(
                    Fore.LIGHTGREEN_EX
                    + "Saving to config/api_config.yml (you can modify the info here)"
                )
                try:
                    ...
                    print(
                        "Could not save configs to file, you will have to type them again later"
                    )

        except Exception as e:
            raise IOError(
                "Corrupted api_config, please re download the file: " + str(e)
            )

        if (
            self.api_config.host is None
            or self.api_config.api_key is None
            or self.api_version is None
        ):
            raise TypeError(
                (
                    "API Configuration values on {} are NOT VALID".format(
    ...
    def request_computers(self):
        ...
        try:
            while True:
                paged_response = api_instance.search_computers(
                    self.api_version,
                    expand=self.COMPUTER_EXPAND,
                    overrides=False,
            ...
            raise Exception(
                "Generic connection error!" + str(e) + "Generic connection error!"
            )
    ...
    def request_computer_per_id(self, comp_id):
        ...
        try:
            api_response = api_instance.describe_computer(
                comp_id, self.api_version, expand=self.COMPUTER_EXPAND, overrides=False
            )
        except deepsecurity.rest.ApiException as e:
            self.handle_api_exception(e, "COMPUTERS")
        except urllib3.exceptions.MaxRetryError as e:
    ...
    def request_computer_groups(self):
        api_instance = deepsecurity.ComputerGroupsApi(
            deepsecurity.ApiClient(self.api_config)
        )
        try:
    ...
    def request_stateful_configs(self):
        ...
        api_instance = deepsecurity.StatefulConfigurationsApi(
            deepsecurity.ApiClient(self.api_config)
        )
        try:
    ...
    def handle_api_exception(self, e, resource_name="RESOURCE"):
        ...
        if e.status == 401:
            ...
            exception_str = (
                "ERROR 403: UNAUTHORIZED ACCESS TO {} - Please allow VIEW access for the key's role".format(
                    resource_name.upper()
                ),
            )
        elif e.status == 404:
